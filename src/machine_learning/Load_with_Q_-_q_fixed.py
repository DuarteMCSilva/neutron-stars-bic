...
dot = int(200)
M_chirp  = np.arange(1.00, 1.35, 0.35/dot) 
q        = np.full(dot**2 , q_mean)
...
M_chirp, Lambda_T = np.meshgrid(M_chirp, Lambda_T)  
########################################################################
...
'''------------------------------------------------------------------'''
M_chirp  = UNNORMALIZE(M_chirp , M_chirp_mean, M_chirp_std)
